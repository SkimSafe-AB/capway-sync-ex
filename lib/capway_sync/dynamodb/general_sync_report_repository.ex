defmodule CapwaySync.Dynamodb.GeneralSyncReportRepository do
  ...
  def store_report(%GeneralSyncReport{} = report) do
    ...
    Logger.info("Storing GeneralSyncReport to DynamoDB table: #{table_name}, report_id: #{report_id}")
  ...
  def list_recent_reports(opts \\ []) do
    ...
    Logger.debug("Scanning GeneralSyncReport table: #{table_name} with params: #{inspect(scan_params)}")
  ...
  defp struct_to_dynamodb_item(%GeneralSyncReport{} = report, report_id) do
    %{
      ...
      # Store detailed lists as JSON strings to avoid DynamoDB nested object limits
  ...
  defp format_datetime_for_dynamodb(datetime) when is_binary(datetime) do
    datetime  # Assume it's already in ISO8601 format
  end
