defmodule Mix.Tasks.SeedData do
  ...
  def run(args) do
    ...
    unsuspend_count =
      Enum.count(test_data, fn sub ->
        ...
        unpaid_invoices_ok =
          case parse_int_safely(sub.unpaid_invoices) do
            {:ok, 0} -> true
            _ -> false
          end
    ...
    Mix.shell().info(
      "   • Should be unsuspended (collection=0 & unpaid_invoices=0): #{unsuspend_count}"
    )
  ...
  def parse_soap_response do
    ...
    Enum.map(report_results, fn [report_result] ->
      ...
      values =
        Regex.scan(
          ~r/<Value(?:\s+[^>]*)?>([^<]*)<\/Value>|<Value[^>]*nil="true"[^>]*\/?>/,
          report_result
        )
        |> Enum.map(fn
    ...
    # Remove duplicates by customer_ref
    |> Enum.uniq_by(& &1.customer_ref)
  end
  ...
  defp random_collection_value do
    ...
    case :rand.uniform(100) do
      # 20% - candidates for unsuspend
      n when n <= 20 -> "0"
      # 15% - normal
      n when n <= 35 -> "1"
      # 20% - candidates for suspend
      n when n <= 55 -> "2"
      # 20% - candidates for suspend
      n when n <= 75 -> "3"
      # 10% - candidates for suspend
      n when n <= 85 -> "4"
      # 10% - candidates for suspend
      n when n <= 95 -> "5"
      # 5% - nil values for edge case testing
      _ -> nil
    end
  end
  ...
  defp random_unpaid_invoices_value do
    ...
    case :rand.uniform(100) do
      # 40% - candidates for unsuspend when collection=0
      n when n <= 40 -> "0"
      # 20% - normal
      n when n <= 60 -> "1"
      # 15% - normal
  ...
  defp generate_swedish_name(existing_names) do
    ...
    case :rand.uniform(3) do
      # Use real name
      1 -> Enum.random(existing_names)
      # Generate simple combination
      2 -> "#{Enum.random(first_names)} #{Enum.random(last_names)}"
      # Swedish pattern with middle name
      3 -> "#{Enum.random(first_names)} #{Enum.random(first_names)} #{Enum.random(last_names)}"
    end
  end
  ...
  defp generate_contract_ref do
    case :rand.uniform(3) do
      1 -> "contract_-#{:rand.uniform(999_999_999_999_999_999)}"
      2 -> generate_uuid()
  ...
  defp generate_date_near(base_date) when is_binary(base_date) do
    ...
    case Date.from_iso8601(base_date) do
      {:ok, date} ->
        # ±30 days
        days_offset = :rand.uniform(60) - 30
  ...
  def insert_to_trinity_db(test_data, suspend_some) do
    ...
    case ensure_trinity_repo_started() do
      :ok ->
        ...
        result =
          Enum.reduce_while(test_data, inserted_count, fn capway_sub, acc ->
            try do
              TrinityRepo.transaction(fn ->
                ...
                subscription_changeset =
                  Subscription.changeset(%Subscription{}, subscription_attrs)
                ...
                case TrinityRepo.insert(subscription_changeset) do
                  {:ok, inserted_subscription} ->
                    ...
                    case TrinityRepo.insert(subscriber_changeset) do
                      ...
                      {:error, changeset} ->
                        Mix.shell().error(
                          "Failed to insert subscriber: #{inspect(changeset.errors)}"
                        )
                  ...
                  {:error, changeset} ->
                    Mix.shell().error(
                      "Failed to insert subscription: #{inspect(changeset.errors)}"
                    )
  ...
  defp determine_payment_method(capway_sub, _inserted_count) do
    ...
    case seed do
      # 70% - will exist in both systems
      n when n < 70 -> "capway"
      # 15% - missing from Capway, should be added
      n when n < 85 -> "bank"
      # 10% - missing from Capway, should be added
      n when n < 95 -> "card"
      # 5% - missing from Capway, should be added
      _ -> "other"
    end
  end
end
