...
### Added
-   **Language-code drift detection for `:capway_update_customer`.** The REST customer sync now checks the Capway customer's `languageCode` against the expected value for the active market and emits an update (`:update_language`) when it drifts.
    -   New `CapwaySync.Market` module — the single source of per-market *business* settings (selected by the existing `MARKET` env var). `language_code/0,1` maps `:se → "sv"` and `:no → "nb"`; unknown markets return `nil` and are never flagged. Adding a market or a per-market setting is a one-line edit to its `@settings` map. (Deployment-level config — credentials, hosts, table names — intentionally stays in env vars.) `CompareDataV2.valid_national_id?/1` now reads the active market through `Market.current/0` too.
    -   `FetchCapwayEmails` now backfills `language_code` from the REST customer record (`languageCode`/`language_code`/`LanguageCode`), alongside `email` and `national_id`. To distinguish "fetched but blank" from "never fetched", a successful fetch records `""` when the record carries no language, while a failed/skipped fetch leaves `nil`.
    -   `Canonical` gains a tri-state `language_code` field: `nil` = not fetched (unknown → ignored, so a transient REST failure can't trigger a false update), `""` = fetched-but-blank (wrong → triggers an update to the market default), value = the fetched code (compared case-/trim-insensitively against the market default).
    -   `CompareDataV2.get_customers_to_update/3` flags a language mismatch on top of the existing national-id/email checks, subject to the same `collection < 2` cap and `capway_sync_excluded` gating.
